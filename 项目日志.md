...
与导师交流，目前考虑可能的两种处理思路。本质上都是去抓住圆形气泡的特征：
...
为了获得下面这样的标注效果我调试了包括模板延展半径，match阈值以及霍夫圆检测的半径阈值多个参数才得到这样的结果：
...
更进一步的，模板的大小我直接使用hough变换找到的圆的半径，不进行超参数上的扩充了。
...
我在考虑如何确定一个较优的阈值，在前序通过多次的模板迭代，最优的match值已经被提升到了一个较为严格的程度，也正是因为这个严格的阈值，导致高p低r。我的电光火石的想法是基于这个严格的阈值，以一个较低的步距在循环中不断的降低它，每一次都检查 经过 去重后剩下的标注框的数量有没有大于我们找到的圆的数量（前面已经验证了霍夫变换在这张图上可以较为准确的找准圆的数量）。
...
2.对于圆形的检测hough变换的调参，实在是麻烦，这批图我很难用一个参数去实现全局的最优，每张图像都要做一些微调。希望上深度来解决这个问题

其次，在框架中我强行要求在最优的阈值上再多找100个圆，因为以一个严格的阈值，recall确实是不够看的，虽然precision挺不错。

这又涉及到一个取舍，对于我们材料学院的任务，更加关注的是precision还是recall呢？
...
第一是在大图上截取我觉得清晰的部分，用之前的代码框架运行出结果之后，把圆的坐标半径信息，按照yolo的txt格式存储起来，（都转化成在原图中的比例），类别都设置为0 。随便搞了20张数据集。

关于使用神经网络，我很熟悉yolov5的使用，就直接先用yolo试试效果。数据集量太小，试试效果，我没有设置验证集。

我的卡是：NVIDIA GeForce RTX 3060 Laptop GPU, 6144MiB
...
这对我而言是宝贵的一次组会，研究并解决问题不完全是猜测，科学性的分析，针对性的优化是正解。
...
五一例行组会，对于数据的认知还是有一些native， 对于特征分析：
